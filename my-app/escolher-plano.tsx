...
const PLANOS = [
  ...
  },
  {
    ...
    preco: 'R$ https://kav-class-1.onrender.com,98',
    periodo: ' anuidade',
    limite: 'Até 70 alunos',
    descricao: 'Pague uma vez uma vez ao ano.',
    recomendado: false,
    features: [
        ...
        >
          ...
          >
            {carregando === plano.id || verificando ? (
              ...
              <>
                <Ionicons
                  name={plano.id === 'one-time' ? 'infinite-outline' : 'flash'}
                  size={16}
                  color={plano.recomendado ? '#fff' : CORES.acento}
                  style={{ marginRight: 6 }}
                />
                <Text style={[styles.btnTexto, plano.recomendado ? styles.btnTextoDestaque : styles.btnTextoNormal]}>
                  {plano.id === 'one-time' ? 'Comprar Acesso Vitalício' : 'Assinar Agora'}
                </Text>
              </>
            )}
          </TouchableOpacity>
