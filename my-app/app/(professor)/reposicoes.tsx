...
export default function ReposicoesScreen() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const [resAlunos, resRepos] = await Promise.all([
        ...
        fetchComRetry(`${API_URL}/api/reposicoes?professorId=${professorId}`, {
          headers: { 'Authorization': `Bearer ${token}` },
        }),
      ]);
  ...
  // Android: spinner inline — atualiza enquanto gira, confirma com botão
  ...
  const enviarSolicitacao = async () => {
    ...
    try {
      ...
      const resposta = await fetchComRetry(`${API_URL}/api/reposicoes`, {
        method: 'POST',
        headers: {
      ...
      <View style={styles.cardForm}>
        ...
        {Platform.OS === 'android' && mostrarPickerData && (
          <View style={styles.containerRoleta}>
            ...
            />
            <TouchableOpacity style={styles.botaoOkRoleta} onPress={confirmarDataAndroid}>
              <Text style={styles.textoOkRoleta}>Próximo →</Text>
            </TouchableOpacity>
          </View>
        )}
        ...
        {Platform.OS === 'android' && mostrarPickerHora && (
          <View style={styles.containerRoleta}>
            <DateTimePicker
              ...
              is24Hour
            />
            <TouchableOpacity style={styles.botaoOkRoleta} onPress={confirmarHoraAndroid}>
              <Text style={styles.textoOkRoleta}>Confirmar</Text>
            </TouchableOpacity>
          </View>
        )}
