// Configurações do professor INSTITUTION — foto, telefone, senha. Mesmo
// GET/PUT /api/professor/perfil do SELF (sem seções de assinatura/PIX/
// link de cartão, que não fazem sentido pra um professor de escola). Como
// é a mesma linha de Professor lida por (escola)/equipe.tsx, qualquer
// alteração aqui já reflete automaticamente no painel institucional.
...
export default function ConfiguracoesProfessorEscola() {
  const { nome, fotoUrl, escolaNome, telefone, carregando, recarregarPerfil, sair } = useProfessorEscolaContexto();
  ...
  const salvarFoto = async (novaFotoUrl: string) => {
    try {
      ...
      await fetchComRetry(`${BASE_URL}/api/professor/perfil`, {
        method: 'PUT',
