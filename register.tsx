...
export default function RegisterScreen() {
  ...
  const aplicarMascaraData = (texto: string) => {
    ...
    else if (valor.length >= 3) return `${valor.slice(0, 2)}/${valor.slice(2)}`;
    return valor;
  };
  ...
  const aplicarMascaraTelefone = (texto: string) => {
    ...
    if (valor.length >= 3) {
      if (valor.length >= 8) return `(${valor.slice(0, 2)}) ${valor.slice(2, 7)}-${valor.slice(7)}`;
  ...
  const finalizarCadastro = async () => {
    ...
    try {
      ...
      const payload = papel === 'professor' 
        ? { nome, email: email.toLowerCase().trim(), senha, telefone, dataNascimento, cursos: cursosSelecionados }
        : { nome, email: email.toLowerCase().trim(), senha, telefone, dataNascimento, codigoConvite: codigoConvite.toUpperCase().trim() };
      ...
      try {
        ...
        console.error("❌ Erro ao ler resposta do servidor:", textoBruto);
        Alert.alert("Erro no Servidor", "O servidor respondeu com um formato inesperado. Tente novamente.");
        return;
      }

      if (resposta.ok) {
        if (papel === 'professor') {
          Alert.alert("Bem-vindo!", `Sua conta foi criada!\nSeu Código é: ${dados.codigoConvite || ''}`);
        } else {
          Alert.alert("Sucesso!", "Conta de aluno criada com sucesso!");
        }
        setCadastroEnviado(true);
      } else {
        Alert.alert("Atenção", dados.erro || "Não foi possível criar a conta.");
      }

    } catch (erro) {
      // Se a internet falhar ou o simulador bloquear, cai aqui, sem tela vermelha!
      console.log("🚨 ERRO DE REDE:", erro);
      Alert.alert(
        "Sem Conexão", 
        "Não conseguimos alcançar o servidor.\nVerifique a internet do seu simulador ou celular e tente novamente."
      );
    }
  };
...
const styles = StyleSheet.create({
  ...
  input: { width: '100%', height: 50, backgroundColor: '#ebebeb', borderRadius: 8, paddingHorizontal: 15, color: '#000000', marginBottom: 15 },
  ...
  roleButton: { flex: 1, height: 50, backgroundColor: '#ebebeb', borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginHorizontal: 5 },
  roleButtonActive: { backgroundColor: '#000000' },
  roleText: { color: '#888', fontWeight: 'bold' },
  roleTextActive: { color: '#ffffff' },
  codigoContainer: { marginBottom: 5 },
  dropdownContainer: { marginBottom: 15 },
  dropdownButton: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#ebebeb', height: 50, borderRadius: 8, paddingHorizontal: 15 },
  dropdownButtonText: { color: '#888', fontSize: 15 },
  dropdownButtonTextSelected: { color: '#000000', fontWeight: 'bold' },
  dropdownBody: { backgroundColor: '#FAFAFA', borderWidth: 1, borderColor: '#ebebeb', borderBottomLeftRadius: 8, borderBottomRightRadius: 8, padding: 15, marginTop: -5 },
  categoriaContainer: { marginBottom: 15 },
  categoriaTitulo: { fontSize: 14, fontWeight: 'bold', color: '#000000', backgroundColor: '#ebebeb', paddingVertical: 4, paddingHorizontal: 8, borderRadius: 4, marginBottom: 10, alignSelf: 'flex-start' },
  itemCurso: { flexDirection: 'row', alignItems: 'center', marginBottom: 12, paddingLeft: 5 },
  checkbox: { width: 22, height: 22, borderRadius: 6, borderWidth: 1.5, borderColor: '#000000', marginRight: 10, alignItems: 'center', justifyContent: 'center' },
  checkboxSelected: { backgroundColor: '#000000' },
  textoCurso: { fontSize: 15, color: '#333' },
  button: { width: '100%', height: 50, backgroundColor: '#000000', borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginTop: 10 },
  buttonText: { color: '#ffffff', fontSize: 18, fontWeight: 'bold' },
  backButton: { marginTop: 20, alignItems: 'center' },
  backButtonText: { color: "#666666", fontSize: 16 },
  successContainer: { flex: 1, backgroundColor: "#ffffff", alignItems: "center", justifyContent: "center", padding: 20 },
