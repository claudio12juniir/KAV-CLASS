...
export default function FinanceiroProfessorScreen() {
  ...
  const carregarDados = async () => {
    try {
      ...
      const [resPag, resPerfil] = await Promise.all([
        fetch(`${API_URL}/api/pagamentos?professorId=${professorId}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch(`${API_URL}/api/professor/perfil?professorId=${professorId}`, {
          headers: { Authorization: `Bearer ${token}` },
        }),
      ]);
  ...
  const mensalidadesMes = mensalidades.filter(m => {
  ...
  const confirmarPagamento = async (id: string, nomeAluno: string) => {
    try {
      const token = await SecureStore.getItemAsync('kav_token');
      const res = await fetch(`${API_URL}/api/pagamentos/${id}/aprovar`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const notificarAluno = async (id: string, nomeAluno: string) => {
    ...
    try {
      const token = await SecureStore.getItemAsync('kav_token');
      const res = await fetch(`${API_URL}/api/pagamentos/${id}/notificar-vencimento`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
